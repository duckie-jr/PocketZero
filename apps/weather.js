...
AppRegistry.register({import { AppRegistry } from './registry.js';
import { Router } from './router.js';
import { Store } from './store.js';
import { Notify } from './notify.js';
import {
...
function renderWeather(container) {
    ...
    if (savedLocation) {
        ...
        if (cachedWeatherData) {
            ...
            displayWeather(cachedWeatherData, savedLocation.name, toDisplayTemp, unitLabel, onUnitToggle);
        } else {
    ...
    function onUnitToggle() {
        ...
        if (cachedWeatherData && location) {
            displayWeather(cachedWeatherData, location.name, toDisplayTemp, unitLabel, onUnitToggle);
        }
    }
...
function displayWeather(data, cityName, toDisplayTemp, unitLabel, onUnitToggle) {
    const body = document.getElementById('weather-body');
    if (!body) return;
